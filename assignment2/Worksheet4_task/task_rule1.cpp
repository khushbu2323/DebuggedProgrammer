...
cout << "Initial Stage:"<<endl;
    cout<<"A living cell with less than two living neighbours dies"<<endl;
...
int main()
{
    ...
    InitializationofGrid = new int*[Game_Board_Row];
	 RuleonefinalGrid = new int*[Game_Board_Row];
    for(i=0; i<Game_Board_Row; i++){
        InitializationofGrid[i] = new int[Game_Board_Col];
		RuleonefinalGrid[i] = new int[Game_Board_Col];
    }
InitialiseGameOfLife(Game_Board_Row,Game_Board_Col,InitializationofGrid);

FinalGame(Game_Board_Row,Game_Board_Col,Live_neighbour,InitializationofGrid,RuleonefinalGrid);
